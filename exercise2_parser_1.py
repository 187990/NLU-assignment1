...
# -*- coding: utf-8 -*-
"""
Created on Mon Apr 19 17:18:10 2021
...
#NEW FEATURES ADDED
            token = self._tokens[stack_idx0]
            if self._check_informative(token["word"], True) and  self._check_informative(num_left):
                result.append("STK_0_WORDNLEF_" + token['word'] + str(num_left))
            if self._check_informative(token["tag"]) and  self._check_informative(num_left):
                result.append("STK_0_TAGNLEF_" + token['tag'] + str(num_left))
            if self._check_informative(token["word"], True) and  self._check_informative(num_right):
                result.append("STK_0_WORDNRIG_" + token['word'] + str(num_right))
            if self._check_informative(token["tag"]) and  self._check_informative(num_right):
                result.append("STK_0_TAGNRIG_" + token['tag'] + str(num_right))  
...
#NEW FEATURE NOT ADDED               
#                if "lemma" in token and self._check_informative(token["lemma"]):
#                    result.append("BUF_3_LEMMA_" + token["lemma"])
#                if "feats" in token and self._check_informative(token["feats"]):
#                    feats = token["feats"].split("|")
#                    for feat in feats:
#                        result.append("BUF_3_FEATS_" + feat)   

#NEW FEATURE ADDED
            if len(self.stack) > 4:
                stack_idx4 = self.stack[len(self.stack) - 4]
                token = self._tokens[stack_idx4]
                if self._check_informative(token["tag"]):
                    result.append("STK_4_POS_" + token["tag"])
                    

#NEW FEATURE NOT ADDED
#            if len(self.stack) > 5:
#                stack_idx5 = self.stack[len(self.stack) - 5]
#                token = self._tokens[stack_idx5]
#                if self._check_informative(token["tag"]):
#                    result.append("STK_5_POS_" + token["tag"])
            ...
            token = self._tokens[buffer_idx0]
            if self._check_informative(token["word"], True) and  self._check_informative(num_left):
                result.append("BUF_0_WORDNLEF_" + token['word'] + str(num_left))
            if self._check_informative(token["tag"]) and  self._check_informative(num_left):
                result.append("BUF_0_TAGNLEF_" + token['tag'] + str(num_left))
            if self._check_informative(token["word"], True) and  self._check_informative(num_right):
                result.append("BUF_0_WORDNRIG_" + token['word'] + str(num_right))
            if self._check_informative(token["tag"]) and  self._check_informative(num_right):
                result.append("BUF_0_TAGNRIG_" + token['tag'] + str(num_right))  
                
 #OTHER FEATURES ADDED
        
        #one word feature on stack
        if len(self.stack) > 0:
            # Stack 0
            token = self._tokens[stack_idx0]
            if self._check_informative(token["word"], True) and self._check_informative(token["tag"]):
                result.append("STK_0_FORMPOS_" + token["word"]+token["tag"])
...
#                    result.append("STK_1_FORMPOS_" + token["word"] + token["tag"])


        # one word feature on buffer
        if len(self.buffer) > 2:
            token = self._tokens[buffer_idx2]
            if self._check_informative(token["word"], True):
...
#                result.append("BUF_3_FORM_" + token["word"])
        ...
        #two word feature on stack
#FEATURES NOT ADDED
#        if len(self.stack) > 0:
            # Stack 0
#            token1 = self._tokens[stack_idx0]
#            if len(self.stack) > 1:
#                token2 = self._tokens[stack_idx1]
#                if self._check_informative(token1["word"], True) and self._check_informative(token2["word"], True):
#                    result.append("STK_12_FORM_" + token1["word"]+token2["word"])
#                if self._check_informative(token1["tag"]) and self._check_informative(token2["tag"]):
#                    result.append("STK_12_TAG_" + token1["tag"]+token2["tag"])        
#                if self._check_informative(token1["tag"]) and self._check_informative(token2["word"], True) and self._check_informative(token2["tag"]):
#                    result.append("STK_12_TAGWORDTAG_" + token1["tag"]+token2["word"]+token2["tag"])        
#                if self._check_informative(token1["tag"]) and self._check_informative(token1["word"], True) and self._check_informative(token2["tag"]):
#                    result.append("STK_12_WORDTAGTAG_" + token1["word"]+token1["tag"]+token2["tag"])        
#                if self._check_informative(token1["word"]) and self._check_informative(token2["word"], True) and self._check_informative(token2["tag"]):
#                    result.append("STK_12_WORDWORDTAG_" + token1["word"]+token2["word"]+token2["tag"])        

        #two words feature on buffer
        if len(self.buffer) > 0:
            token = self._tokens[buffer_idx0]
#FEATURES NOT ADDED
#            if self._check_informative(token["word"], True) and self._check_informative(token["tag"]):
#                result.append("BUF_0_FORMPOS_" + token["word"]+token["tag"])
#        if len(self.buffer) > 1:
#            token = self._tokens[buffer_idx1]
#            if self._check_informative(token["word"], True) and self._check_informative(token["tag"]):
#                result.append("BUF_1_FORMPOS_" + token["word"]+token["tag"])    
#        if len(self.buffer) > 2:
#            token = self._tokens[buffer_idx1]
#            if self._check_informative(token["word"], True) and self._check_informative(token["tag"]):
#                result.append("BUF_2_FORMPOS_" + token["word"]+token["tag"])    

         #mixing stack and buffer
        if len(self.stack) > 0:
            token = self._tokens[stack_idx0]     
            if len(self.buffer) > 0:
                token_buff = self._tokens[buffer_idx0]     
                if self._check_informative(token["tag"]) and self._check_informative(token_buff["word"], True):
                    result.append("STK_11_TAGWORD_" + token["tag"]+token_buff["word"])                
